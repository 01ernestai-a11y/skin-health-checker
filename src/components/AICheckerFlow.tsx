...
import { analyzeSkinInitial } from '@/app/actions/checker'
...
export interface AICheckerFlowProps {
    saveVerdictAction: (
        photoUrl: string,
        initialReview: string,
        answers: Record<string, string>
    ) => Promise<{ success: true; verdict: string; healthCheckId?: string } | { success: false; error: string }>
}

export default function AICheckerFlow({ saveVerdictAction }: AICheckerFlowProps) {
    ...
    const handleQuestionsSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const result = await saveVerdictAction(photoUrl, initialReview, answers)
            if (result.success) {
                ...
                <Card className="shadow-lg border-0 ring-1 ring-slate-200">
                    ...
                    {step === 'ANALYSIS' && (
                        <CardContent className="py-16 text-center">
                            ...
                            <p className="text-slate-500 mt-2 max-w-sm mx-auto text-sm">{t('analyzingDetail')}</p>
                        </CardContent>
                    )}
                            ...
                            <CardHeader className="bg-indigo-50/50 border-b pb-6">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <CardDescription className="text-indigo-700 mt-1">{t('analysisDetail')}</CardDescription>
                                    </div>
                                </div>
                            </CardHeader>
                                ...
                                <form id="questions-form" onSubmit={handleQuestionsSubmit} className="space-y-5">
                                    {(['q1', 'q2', 'q3', 'q4', 'q5'] as const).map(q => (
                                        <div key={q} className="space-y-3">
                                            <Label className="text-base font-medium">{t(q)}</Label>
                                            <Input
                                                required
                                                value={answers[q]}
                                                onChange={e => setAnswers({ ...answers, [q]: e.target.value })}
                                                placeholder={t(`${q}Placeholder` as any)}
                                            />
                                        </div>
                                    ))}
                                </form>
                    ...
                    {step === 'VERDICT' && (
                        <>
                            ...
                            <CardFooter className="bg-slate-50 border-t p-6 flex flex-col sm:flex-row justify-center gap-3">
                                <Button className="w-full sm:w-auto h-11" onClick={() => setStep('CHAT')}>
                                    <Brain className="mr-2 h-4 w-4" />
                                    {t('askAI')}
                                </Button>
                                <Button variant="outline" className="w-full sm:w-auto h-11" onClick={reset}>
