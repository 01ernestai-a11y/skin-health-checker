    ...
    if (error) {
        return (
            <div className="p-4 bg-red-50 text-red-700 rounded-md">
                Error loading forum posts: {error.message}
            </div>
        )
    }
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Discuss clinical cases, share experiences, and collaborate with verified doctors.
                    </p>
                </div>
                <CreatePostButton />
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
                {posts && posts.length > 0 ? (
                    ...
                    <div className="text-center py-12 px-4 border rounded-lg bg-slate-50 border-dashed">
                        ...
                        <h3 className="text-sm font-medium text-slate-900">No posts yet</h3>
                        <p className="text-sm text-slate-500 mt-1">Starting a discussion to share knowledge with your peers.</p>
                    </div>
                )}
            </div>
        </div>
